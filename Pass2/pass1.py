		...
		if(i == 0):
				...
				if(word == 'using' or word == 'drop'):
					...
					for literals in range(0, len(literaltable)):
						if(literaltable[literals]["value"] == None):
							...
							literaltable[literals]["lineno"] = lineno
							lc += 4
				elif(word == 'end'):
					for literals in range(0, len(literaltable)):
						if(literaltable[literals]["value"] == None):
			...
			if(word in pot):
				...
				if word not in literaltable:
					literaltable.append({"literal": word, "value": None, "length": 4, "ar": "r", "lineno" : None})
				print("in literal table")
			elif(word.isnumeric() or word == '*'):
				print("constant")
		
	if(found == False):
		linetable.append({"lineno" : lineno, "lc" : lcstart})
		lineno += 1
		out.write(line)
...
tables.write("\n")
tables.write("LineTable = " + str(linetable))
